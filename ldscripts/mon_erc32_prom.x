/* Linker script, for executables in PROM */
OUTPUT_FORMAT("ihex")
 ...
 *
 **************************************************************************/
STARTUP("art0.o")   /* use private art0.o */
ENTRY(__cold_start)
/*
...
SECTIONS
{
  ...
  _swall  =  ADDR (.init) + SIZEOF (.init) + SIZEOF (.text) + SIZEOF (.rodata) + SIZEOF(.data);
  _ewall  = 0x8000 - _swall;
  .data0 : AT (_swall) {
    	LONG(0x91D02001); /* ta 1 INT_SEGV*/
    	FILL(0x91D02001); /* ta 1 INT_SEGV*/
    	. = _ewall ; 
  }
  .stab  0 (NOLOAD) :
  {
